...
@section('content')
    ...
    <div class="lg:col-span-3">
        ...
        @else
            {{-- Timer Countdown (fixed ke viewport, tidak ikut ter-scroll) --}}
            @if ($waktuSelesai)
                {{-- Spacer: mengisi ruang yang ditinggalkan karena timer di-fixed
         (dilepas dari document flow). Tinggi disesuaikan dengan tinggi timer. --}}
                <div id="quiz-timer-spacer" class="mb-6 h-[76px] sm:h-[84px]"></div>

                <div id="quiz-timer" data-deadline="{{ $waktuSelesai->toIso8601String() }}"
                    class="fixed inset-x-0 top-0 z-50 mx-auto flex max-w-2xl items-center justify-between
               border border-line bg-white px-6 py-4 shadow-sm
               rounded-b-2xl sm:top-4 sm:rounded-2xl"
            ...
            <form id="quiz-form" method="POST" action="{{ route('student.quiz.submit', $quiz->id) }}">
                ...
                <div class="mb-6 rounded-2xl border border-line bg-white p-6 shadow-sm">
                    <h1 class="font-display text-xl font-bold text-ink">{{ $quiz->judul }}</h1>
                    @if ($quiz->deskripsi)
                        ...
                        <div class="rounded-2xl border border-line bg-white p-6 shadow-sm">
                            ...
                            <div class="mt-4 space-y-2">
                                @foreach ($q->pilihanTersedia() as $huruf => $teks)
                                    <label
                                        class="flex cursor-pointer items-center gap-3 rounded-lg border border-line
                                      ...
                                      has-[:checked]:border-ink has-[:checked]:bg-paper has-[:checked]:font-semibold">
                                        <input type="radio" name="jawaban[{{ $q->id }}]"
                                            value="{{ $huruf }}" class="h-4 w-4 quiz-radio" required>
