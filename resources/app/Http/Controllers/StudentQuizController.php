...
class StudentQuizController extends Controller
{
    public function show(Quiz $quiz)
    {
        ...
        $terdaftar = \App\Models\PengajaranMahasiswa::where('kelas_id', $quiz->pengajaranDosen->kelas_id)
        ...
        if ($quiz->durasi_menit && !session()->has($sessionKey)) {
    ...
    public function submit(Request $request, Quiz $quiz)
    {
        ...
        $sessionKey = 'quiz_mulai_' . $quiz->id . '_' . $mahasiswa->id;
        if ($quiz->durasi_menit && session()->has($sessionKey)) {
            $deadline = \Carbon\Carbon::parse(session($sessionKey))->addMinutes($quiz->durasi_menit);
            // Beri toleransi kecil untuk latency request, tetapi jangan menerima submission lama.
            if (now()->greaterThan($deadline->addSeconds(10))) {
                $validated['jawaban'] = $validated['jawaban'] ?? [];
            }
        }
