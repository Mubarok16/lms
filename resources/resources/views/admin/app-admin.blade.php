...
<body class="font-sans text-ink antialiased">
    ...
    <div x-data="{ sidebarOpen: false, courseOpen: true }" class="min-h-screen flex">
        ...
        <!-- ============ OVERLAY (mobile) ============ -->
        <div x-show="sidebarOpen" x-cloak @click="sidebarOpen = false" class="fixed inset-0 bg-ink/40 z-30 lg:hidden">
        </div>
        <!-- ============ SIDEBAR ============ -->
                        ...
                        <ul class="text-sm space-y-1 list-disc list-inside">
                            @if (is_array(session('error')))
                                @foreach (session('error') as $error)
                                    <li>
                                        {{ isset($error['row']) ? 'Baris '.$error['row'].': ' : '' }}
                                        {{ isset($error['errors']) ? implode(', ', (array) $error['errors']) : (is_string($error) ? $error : json_encode($error)) }}
                                    </li>
                                @endforeach
                            @else
                                <li>{{ session('error') }}</li>
